...
class JacShell:
    ...
    def multTranspose(self, A, X, Y):
        self.x_tensor = torch.from_numpy(X.getArray(readonly=True).reshape(self.ode_.u_tensor.size())).type(torch.cuda.FloatTensor)
        y = Y.array
...
class ODEPetsc(object):
    ...
    def evalFunction(self, ts, t, U, F):
        f = F.array
        self.u_tensor = torch.from_numpy(U.getArray(readonly=True).reshape(self.u_tensor.size())).type(torch.cuda.FloatTensor)#.to(self.u_tensor.device)
        dudt = self.func(t, self.u_tensor).cpu().detach().numpy()
    #    dudt = self.func(t, self.u_tensor).detach().numpy()
        f[:] = dudt.flatten()

    def evalJacobian(self, ts, t, U, Jac, JacPre):
        """Cache t and U for matrix-free Jacobian """
        self.t = t
        self.u_tensor = torch.from_numpy(U.getArray(readonly=True).reshape(self.u_tensor.size())).type(torch.cuda.FloatTensor)
    ...
    def saveSolution(self, ts, stepno, t, U):
        ...
        if True:#abs(t-self.sol_times[self.cur_index]) < 1E-16:
            unew = torch.from_numpy(U.getArray(readonly=True).reshape(self.u_tensor.size())).type(torch.cuda.FloatTensor)
      ...
      #  U = PETSc.Vec().createWithArray(u0.numpy()) # convert to PETSc vec
        ...
        for j0 in range(len(solution)-1):
            ...
            while j < len(t) and t1 > t[j] - self.step_size/1000:# and t1 > t0:
                #print(t1,t[j])
                sol_interp.append(self._linear_interp(t0,t1,u00,u1,t[j]).to(u0.device))
                j += 1
    ...
    def petsc_adjointsolve(self, t):
        ...
        adj_u_tensor = torch.from_numpy(adj_u[0].getArray().reshape(self.u_tensor.size())).type(torch.cuda.FloatTensor)#.to(self.u0.device)
        adj_p_tensor = torch.from_numpy(adj_p[0].getArray().reshape(self.np)).type(torch.cuda.FloatTensor)#.to(self.u0.device)
    ...
    def backward(ctx, *grad_output):
        ...
        with torch.no_grad():
            ...
            for i in range(T-1, 0, -1):
                adj_u_tensor, adj_p_tensor = ctx.ode.petsc_adjointsolve(torch.tensor([t[i], t[i - 1]]))#.to(grad_output[0].device))
                adj_u_tensor = adj_u_tensor + grad_output[0][i-1]#.cpu() # add forcing
        adj_u_tensor, adj_p_tensor = adj_u_tensor.to(grad_output[0].device), adj_p_tensor.to(grad_output[0].device)
