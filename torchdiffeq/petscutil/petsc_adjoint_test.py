...
class ODEPetsc(object):
    ...
    def evalFunction(self, ts, t, U, F):
        f = F.array
        self.u_tensor = torch.from_numpy(U.getArray(readonly=True).reshape(self.u_tensor.size())).type(torch.FloatTensor).to(self.u_tensor.device)
    ...
    def petsc_adjointsolve(self, t):
        ...
        adj_u_tensor = torch.from_numpy(adj_u[0].getArray().reshape(self.u_tensor.size())).type(torch.FloatTensor)#.to(self.u0.device)
        adj_p_tensor = torch.from_numpy(adj_p[0].getArray().reshape(self.np)).type(torch.FloatTensor)#.to(self.u0.device)
